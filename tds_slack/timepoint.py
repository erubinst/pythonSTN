...
class Timepoint:
    ...
    def add_constraint(self, other, constraint_type, min_gap=0, max_gap=np.inf, print_inconsistencies=True, return_affected_timepoint=False):
        """
        Add: min_gap <= other - self <= max_gap
        Uses STN.add_constraint(self.name, other.name, lb, ub)
        """
        return self.tds.stn.add_constraint(self.name, other.name, lb=min_gap, ub=max_gap, constraint_type=constraint_type, print_inconsistencies=print_inconsistencies, return_affected_timepoint=return_affected_timepoint)
    ...
    def delete_constraint(self, other, constraint_type):
        ...
        """
        return self.tds.stn.delete_constraint(self.name, other.name, constraint_type)
